...
> **Auth error responses (deferred, see §14):** `POST /auth/login` returns a single generic `401 UNAUTHORIZED` with message `"Authentication failed"` for every login failure — wrong password, unknown email, and disabled account (`CmsUserDetails.isEnabled() == false`) all produce the same message. Spring Security already distinguishes these internally (`BadCredentialsException` vs `DisabledException`, both `AuthenticationException` subtypes) — the generic response is a deliberate OWASP-aligned choice to avoid user enumeration on a public endpoint with no rate limiting yet. Revisit once `/auth/**` has rate limiting (§11) and abuse mitigation in place; a disabled account should then be communicated out-of-band (e.g. notification email), not via a more specific login error.
>
> The same `GlobalExceptionHandler.handleAuthentication` also catches `AuthenticationException` thrown for an invalid/missing JWT on `/cms/**` — a *different* situation from a failed login. The message is deliberately worded generically ("Authentication failed", not "Invalid email or password") so it's accurate in both cases without a second handler.

> **Password minimum length (8 characters):** follows NIST SP 800-63B and the OWASP Authentication Cheat Sheet — both treat length as the dominant factor in password strength and explicitly advise against forced composition rules (mandatory uppercase/digit/symbol), which empirically push users toward predictable patterns (`Password1!`) rather than stronger passwords. 8 is the accepted industry floor, not a target — if a stricter policy is ever needed, raise the minimum (10–12) rather than adding composition rules. Enforced via `@Size(min = 8)` on `RegisterRequestDTO`.
